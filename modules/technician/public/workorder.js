...
function validateCompletion() {
  ...
  els.btnSaveSig.addEventListener('click', () => {
    ...
    const nameTrim = ((els.signerName && els.signerName.value) || '').trim()
      || (draft.signoff.signerName || '').trim();
    if (!nameTrim) {
      alert('Enter the signer name on the Sign-off tab before saving the signature');
      return;
    }
    ...
    saveDraft(draft);
    updateSigStatus();
    const sat = parseInt((els.satisfactionRating && els.satisfactionRating.value) || draft.signoff.signerSatisfaction || '0', 10) || 0;
    const fb = ((els.satisfactionFeedback && els.satisfactionFeedback.value) || draft.signoff.signerFeedback || '').trim();
    ...
    window.MRTS.offline.queueAction('signature_save', woIdNum > 0 ? woIdNum : woId, {
      ...
      signer_satisfaction: sat > 0 ? sat : undefined,
      signer_feedback: fb || undefined,
    });
    updateCompletionBlocker();
    renderChecklistTimeLogs(); // refresh signature status in checklist panel
    if (typeof updateCompletionBadges === 'function') updateCompletionBadges();
    ...
    try {
      ...
      if (window.MRTS.offline && window.MRTS.offline.queueCount() > 0) {
        ...
        try {
          ...
          els.btnComplete.disabled = false;
          els.btnComplete.innerHTML = originalText;
          alert('Could not sync pending changes — check your connection and try again.\n\n' + (syncErr.message || syncErr));
          return;
        }
  ...
  async function load() {
    ...
    if (wo.notes && wo.notes.length > 0) {
      ...
      wo.notes.forEach((n) => {
        ...
        if (!text || localTexts.has(text)) return;
        draft.notes.push({
          id: 'srv_' + n.note_id,
          title: '',
          text: n.note_text || '',
  ...
  window.enrichOfflineQueueBeforeSync = function (queueItems) {
    ...
    const name = ((els.signerName && els.signerName.value) || '').trim()
    ...
    return queueItems.map((it) => {
      ...
      if (sat > 0) data.signer_satisfaction = sat;
      if (fb) data.signer_feedback = fb;
  ...
  function mergeServerStateIntoDraft(state) {
    ...
    if (state.signoff && ((state.signoff.signature_path && state.signoff.signature_path !== 'data:inline') || state.signoff.signer_name)) {
      if (state.signoff.signer_name) draft.signoff.signerName = state.signoff.signer_name;
      if (state.signoff.satisfaction != null && state.signoff.satisfaction > 0) {
        draft.signoff.signerSatisfaction = String(state.signoff.satisfaction);
      }
      if (state.signoff.feedback) draft.signoff.signerFeedback = state.signoff.feedback;
      const p = state.signoff.signature_path || '';
      if (p && p !== 'data:inline') draft.signoff.signatureDataUrl = p;
    }

    if (wo && Array.isArray(state.checklist) && wo.checklist && wo.checklist.length) {
      const byId = new Map(state.checklist.map((it) => [it.id, it]));
      wo.checklist.forEach((item) => {
